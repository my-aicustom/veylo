...
import { simulate, speak, transcribe } from '@/lib/client-ai';
import type { Profile } from '@/lib/types';
import { ProfileForm } from '@/components/ProfileForm';

type SimTurn = { id: string; speaker: 'you' | 'ai'; text: string; at: string };
...
export default function SimulationPage() {
    ...
    try {
      ...
      try {
        ...
        await new Promise<void>((resolve) => {
          audio.onended = () => resolve(); audio.onerror = () => resolve(); audio.onpause = () => resolve();
        });
  ...
  async function process(bytes: Uint8Array) {
    ...
    const userTurn: SimTurn = { id: crypto.randomUUID(), speaker: 'you', text, at: new Date().toISOString() };
    ...
    const aiTurn: SimTurn = { id: crypto.randomUUID(), speaker: 'ai', text: result.reply, at: new Date().toISOString() };
  ...
  async function start() {
    ...
    try {
      ...
      const recorder = new PhraseRecorder(stream, {
        silenceMs: 620, minSpeechMs: 280, maxPhraseMs: 5200, threshold: 0.015,
        onPhrase: (phrase) => {
  ...
  return (
    <main className="tool-shell">
      <header className="tool-header"><button className="ghost small" onClick={() => router.push('/')}>← Home</button><div><strong>AI Simulation</strong><span>{status}</span></div></header>
      <section className="tool-grid">
        <div className="setup-card">
          <div className="eyebrow">PRACTICE / INTERNATIONAL CONVERSATION</div>
          <h1>{role}</h1>
          <div className="field-grid">
            <label><span>Counterpart country</span><select value={country} disabled={running} onChange={(e) => setCountry(e.target.value)}>{COUNTRIES.map((c) => <option key={c.code} value={c.code}>{c.name}</option>)}</select></label>
            <label><span>Counterpart language</span><select value={language} disabled={running} onChange={(e) => setLanguage(e.target.value)}>{ALL_LANGUAGE_CODES.map((lang) => <option key={lang} value={lang}>{lang.toUpperCase()}</option>)}</select></label>
          ...
          <div className="route-card"><div><strong>{profile.name}</strong><span>{profile.countryName}</span></div><span>↔ AI</span><div><strong>{role}</strong><span>{countryName(country)} · {language.toUpperCase()}</span></div></div>
          <button className={running ? 'danger' : 'primary'} onClick={running ? stop : start}>{running ? 'End simulation' : 'Start simulation'}</button>
        </div>
        <div className="transcript-card">
          <div className="eyebrow">CONVERSATION</div>
          {turns.length === 0 ? <p className="empty-caption">Your simulated conversation will appear here.</p> : turns.map((turn) => <div className={`turn ${turn.speaker}`} key={turn.id}><span>{turn.speaker === 'you' ? profile.name : role}</span><p>{turn.text}</p></div>)}
        </div>
      </section>
    </main>
  );
