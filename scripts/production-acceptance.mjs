...
Options:
  ...
  --expected-version <v> Require a specific deployed Veylo version
...
async function main() {
  ...
  if (appResponse) {
    ...
    record('CSP blocks framing', /frame-ancestors\s+'none'/i.test(csp), csp || 'missing CSP');
    record('CSP blocks plugins', /object-src\s+'none'/i.test(csp), csp || 'missing CSP');
    const permissions = appResponse.headers.get('permissions-policy') || '';
    record(
      'Permissions policy scopes camera/mic',
      /camera=\(self\)/i.test(permissions) && /microphone=\(self\)/i.test(permissions),
      permissions || 'missing Permissions-Policy',
    );
    if (target.protocol === 'https:') {
      const hsts = appResponse.headers.get('strict-transport-security') || '';
      record('HSTS has max-age', /max-age=\d+/i.test(hsts), hsts || 'missing HSTS');
    }
  }
  ...
  await safe('Readiness endpoint', async () => {
    ...
    record(
      'Readiness cache disabled',
      /no-store/i.test(response.headers.get('cache-control') || ''),
      response.headers.get('cache-control') || 'missing Cache-Control',
    );
    if (options.expectedVersion) {
      record(
        'Expected deployed version',
        readyBody?.version === options.expectedVersion,
        `expected=${options.expectedVersion}; actual=${readyBody?.version || 'missing'}`,
      );
    }
  ...
  await safe('Health endpoint', async () => {
    ...
    if (readyBody?.version && healthBody?.version) {
      record('Runtime version consistency', readyBody.version === healthBody.version, `ready=${readyBody.version}; health=${healthBody.version}`);
    }
    record(
  ...
  await safe('Cross-origin API rejection', async () => {
    const response = await fetchWithTimeout(healthUrl, { headers: { Origin: 'https://invalid-origin.example' } });
    record('Cross-origin API rejection', response.status === 403, `HTTP ${response.status}`);
  });

  await safe('Invalid invite payload rejection', async () => {
    const response = await fetchWithTimeout(inviteUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ roomName: '../bad room' }),
    });
    record('Invalid invite payload rejection', response.status === 400, `HTTP ${response.status}`);
  });
